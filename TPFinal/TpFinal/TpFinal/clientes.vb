...
Public Class clientes
    ...
    End Sub
    'Mostrar clientes con activo=si para datagrid ventasItems
    Public Sub MostrarClientesActivos(tablaClientes As DataGridView)
        Try
            tablaClientes.DataSource = Nothing
            Dim consulta As String = "SELECT Nombre,Telefono FROM clientes where activo='si'"
            Dim adaptador As MySqlDataAdapter = New MySqlDataAdapter(consulta, establecerConexion())
            Dim tabla As DataTable = New DataTable()
            adaptador.Fill(tabla)
            tablaClientes.DataSource = tabla
        ...
        End Try
    End Sub
    'Seleccionar Cliente para VentasItems
    Public Sub SeleccionarClienteAvender(tabla As DataGridView, txtNombre As TextBox)
        Try
        ...
        End Try
    End Sub
    'Buscar Cliente Activo para VentasItems
    Public Sub BuscaClienteActivo(tablaclientes As DataGridView, nombreClienteBuscado As String)


        Try
            tablaclientes.DataSource = Nothing
            Dim consulta As String = "SELECT Nombre,Telefono FROM clientes WHERE Nombre LIKE @nombre AND activo='si'"
            Dim comando As MySqlCommand = New MySqlCommand(consulta, establecerConexion())
            comando.Parameters.AddWithValue("@nombre", "%" & nombreClienteBuscado & "%")
            Dim adaptador As MySqlDataAdapter = New MySqlDataAdapter(comando)
            Dim tabla As New DataTable
    ...
    'Encuentro el id del cliente segun el nombre
    Public Function EncontrarIDCliente(nombreCliente As String) As Integer
        Dim Id_Obtenido As Integer
        Try

            Dim consulta As String = "SELECT id_cliente FROM clientes where Nombre=@Nombre"
            Dim comando As MySqlCommand = New MySqlCommand(consulta, establecerConexion())
            comando.Parameters.AddWithValue("@Nombre", nombreCliente)
            Dim resultado As Object = comando.ExecuteScalar()
            If resultado Is Nothing Then
                MessageBox.Show("Error al encontrar Id del cliente")
                Return 0
            Else
